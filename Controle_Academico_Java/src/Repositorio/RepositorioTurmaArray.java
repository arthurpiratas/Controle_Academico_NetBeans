...
public class RepositorioTurmaArray implements IRepositorioTurma{
	...
	public void listaTurma() {
		...
		for (int i = 0; i < index; i++) {
			System.out.println("Codigo: " + listaTurma[i].getId() +" Nome: " + listaTurma[i].getNome() + " Quantidade: " + listaTurma[i].getQtdAlunoTurma() + " C�digo disciplina: " + listaTurma[i].getDisciplina() + " Professor: " + listaTurma[i].getProfessor());
			for (int j = 0; j < listaTurma[i].getQtdAlunoTurma(); j++) {
				System.out.println("Matr�cula: " + listaTurma[i].getAlunoTurma()[j]);
			}
		}
	...
	// Verifica��o se uma turma possui professores ou disciplina em caso de exclus�o de um professor ou disciplina que est�o em uma turma
	...
	public boolean verificaProfessorPossuiTurma(int idProfessor) {
		...
		for (int i = 0; i < index; i++) {
			if(listaTurma[i].getProfessor() == idProfessor){
                            return true;
                        }
		}
